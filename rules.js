...
for(let i = 0; i<8; i++)
    {
        if(frequenzy()[i]>=2 && pairPoints == 0)
            {
                pairpoints = i*2
            }
        else if(frequenzy()[i]>=2 && pairPoints>1)
            {
                twoPairPoints = pairPoints + (i*2)
            }
...
function chancePoints() {
    let chancePoints = 0;
    for (values of getValues){
        chancePoints=chancePoints+values;
    }
    return chancePoints;
}
